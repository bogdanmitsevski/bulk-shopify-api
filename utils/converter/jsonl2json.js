...
async function convertJSONLtoJSON() {
    fs.readFile('/Users/Bogdan/Desktop/writeFile/file.jsonl', 'utf-8', (err, jsonString) => {
        if (err) {
            console.log(err);
            return;
        }
        ...
        for (let i = 0; i < object.length; i++) {
            ...
            for (let subProperty in subObject) {
                if (subObject["selectedOptions"] === undefined) {
                    console.log(2, subObject);
                    switch (subProperty) {
                        case 'handle':
        ...
        //console.log(object);
        fs.writeFile('/Users/Bogdan/Desktop/writeFile/file1.json', JSON.stringify(object, null, 2), 'utf-8', (err) => {
            if (err) {
                console.log(err);
                return;
            }
            console.log('Data was converted to JSON');
            // fs.unlink('/Users/Bogdan/Desktop/writeFile/file.jsonl', (err) => {
            //     if (err) {
            //         console.log(err);
            //     }
